...
class ClassifierMapper:
    ...
    def createSvm(self, classifier, file_path):
        with open(file_path, mode = 'w',  newline='', encoding='utf-8') as destination_file:
...
#                    csv_writer.writerow([classifier.classes_[i].replace(" ", "-"), classifier.n_support_[i]])
                    csv_writer.writerow([classifier.classes_[i], classifier.n_support_[i]])
            else:
                csv_writer.writerow([str(classifier.classes_[1]).replace(" ", "-"), classifier.n_support_[1]])
                csv_writer.writerow([str(classifier.classes_[0]).replace(" ", "-"), classifier.n_support_[0]])
